...
:: ============================================================
:: ====== MODS CONFIG (AUTO LOAD FROM mods.ini - ONLINE) ======
:: ============================================================
set "PS=%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe"

:: Ưu tiên mods.ini cùng thư mục với installer.bat
set "MODS_CFG=%~dp0mods.ini"
if not exist "%MODS_CFG%" (
  :: fallback nếu ai đó chạy lẻ từ nơi khác
  set "MODS_CFG=%APPDATA%\GTA5S_ModInstaller\mods.ini"
)

call :LOAD_MODS
if errorlevel 1 (
  echo(
  echo [ERR] Khong doc duoc danh sach mod tu mods.ini
  echo File: "%MODS_CFG%"
  pause
  exit /b 1
)
...
for /L %%I in (1,1,%MOD_COUNT%) do (
  if defined MOD_URL_%%I (
    if defined MOD_NAME_%%I (
      echo %%I. !MOD_NAME_%%I!
    ) else (
      echo %%I. Mod so %%I
    )
...
if %CH% GEQ 1 if %CH% LEQ %MOD_COUNT% (
  if not defined MOD_URL_%CH% (
    echo(
    echo [!] Mod nay khong co URL. Vui long kiem tra mods.ini
    timeout /t 2 >nul
    goto :MENU
  )
  cls
  call :PRINT_HEADER
  echo(
  if defined MOD_NAME_%CH% (
    echo Bạn đã chọn: !MOD_NAME_%CH%!
  ) else (
    echo Bạn đã chọn: Mod so %CH%
  )
  call :INSTALL_MOD %CH%
  if errorlevel 1 (
    echo(
...
if not exist "%MODS_CFG%" exit /b 1

set "CUR="
for /f "usebackq tokens=1,* delims==" %%A in ("%MODS_CFG%") do (
  set "K=%%A"
  set "V=%%B"

  if not defined K goto :LOAD_CONT

  :: trim space for key
  for /f "tokens=* delims= " %%t in ("!K!") do set "K=%%t"
  if not defined K goto :LOAD_CONT

  :: ignore comment lines starting with ; or #
  if "!K:~0,1!"==";" goto :LOAD_CONT
  if "!K:~0,1!"=="#" goto :LOAD_CONT

  if /I "!K:~0,1!"=="[" (
    for /f "tokens=1 delims=]" %%X in ("!K:~1!") do (
      set "CUR=%%X"
      :: only digits
      for /f "delims=0123456789" %%Z in ("!CUR!") do set "CUR="
      if defined CUR if !CUR! GTR !MOD_COUNT! set "MOD_COUNT=!CUR!"
    )
  ) else (
    if defined CUR (
      if /I "!K!"=="name" set "MOD_NAME_!CUR!=!V!"
      if /I "!K!"=="url"  set "MOD_URL_!CUR!=!V!"
    )
  )
