...
:: mods.ini path comes from launcher as %1
set "MODS_INI=%~1"
...
call :LOAD_MODS
if errorlevel 1 (
  echo [ERR] Khong doc duoc mods.ini: "%MODS_INI%"
  echo Hay kiem tra file mods.ini tren GitHub / cache.
  echo(
  pause
  exit /b 1
)
...
for /L %%I in (1,1,!MOD_MAX!) do (
  set "N=!MOD_NAME_%%I!"
  if defined N echo %%I. !N!
)
...
:: Validate numeric choice 1..MOD_MAX
set "ISNUM=1"
for /f "delims=0123456789" %%A in ("%CH%") do set "ISNUM=0"
if "%ISNUM%"=="1" (
  if %CH% GEQ 1 if %CH% LEQ !MOD_MAX! (
    cls
    call :PRINT_HEADER
    echo(
...
if not exist "%MODS_INI%" exit /b 1

for /f "usebackq eol=# delims=" %%L in ("%MODS_INI%") do (
  set "LINE=%%L"
  if defined LINE (
...
if not defined MOD_MAX exit /b 1
if "%MOD_MAX%"=="0" exit /b 1

:: Ensure each mod has URL; allow missing name
for /L %%I in (1,1,%MOD_MAX%) do (
  set "U=!MOD_URL_%%I!"
  if not defined U exit /b 1
  set "N=!MOD_NAME_%%I!"
  if not defined N set "MOD_NAME_%%I=Mod so %%I"
)
exit /b 0
