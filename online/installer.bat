...
:: ===================== NHAN THAM SO TU LOADER =====================
:: %1 = path mods.txt (local cache)
:: %2 = APP_NAME
:: %3 = APP_EXE
:: %4 = APP_MANIFEST
set "MODS_LIST_FILE=%~1"
set "APP_NAME=%~2"
set "APP_EXE=%~3"
set "APP_MANIFEST=%~4"

if not defined APP_NAME set "APP_NAME=GTA5S"
if not defined APP_EXE set "APP_EXE=GTA5S.exe"
if not defined APP_MANIFEST set "APP_MANIFEST=GTA5S.VisualElementsManifest.xml"

title %APP_NAME% Cài đặt Mod nhanh trên %APP_NAME% - Install Mods (Coded by KayC)
color A

set "PS=%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe"

echo(
echo %APP_NAME% Cài đặt Mod trên %APP_NAME% - Install Mods (Coded by KayC)
echo(
...
:: ============================================================
:: ===================== MOD LIST (ONLINE) ====================
::  File mods.txt format:  Ten mod|URL
::  Comment line: # ...
:: ============================================================
set "MOD_COUNT=0"

if not exist "%MODS_LIST_FILE%" (
  echo [ERR] Khong tim thay file danh sach mod:
  echo "%MODS_LIST_FILE%"
  pause & exit /b 1
)

for /f "usebackq delims=" %%L in ("%MODS_LIST_FILE%") do (
  set "LINE=%%L"
  if not "!LINE!"=="" (
    if /i not "!LINE:~0,1!"=="#" (
      for /f "tokens=1* delims=|" %%A in ("!LINE!") do (
        ...
        if not "!URL!"=="" (
          set /a MOD_COUNT+=1
          set "MOD_NAME_!MOD_COUNT!=!NAME!"
          set "MOD_URL_!MOD_COUNT!=!URL!"
        )
      )
    )
  )
...
if %MOD_COUNT% lss 1 (
  echo [ERR] File mods.txt khong co dong hop le (Ten|URL).
  pause & exit /b 1
)
...
set "ISNUM=1"
for /f "delims=0123456789" %%A in ("%CH%") do set "ISNUM=0"
if "%ISNUM%"=="0" goto :BAD

if %CH% lss 1 goto :BAD
if %CH% gtr %MOD_COUNT% goto :BAD
...
set "DL_DIR=%TEMP%\%APP_NAME%_MODS_DL"
...
:AFTER_INSTALL
echo(
choice /c YN /m "Bạn có muốn vào game %APP_NAME% ngay không? (Y: vào game / N: quay lại MENU)"
